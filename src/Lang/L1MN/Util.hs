...
module Lang.L1MN.Util where

import UVMHS

import Util.Lex

import Lang.L1MN.Data
...
pExpr = cpNewContext "expression" $ mixfix $ concat
  ...
  , mixInfixL (𝕟64 1) $ do cpSyntax "-" ; return MinusE
  , mixInfixL (𝕟64 2) $ do cpSyntax "*" ; return TimesE
  , mixPrefix (𝕟64 3) $ do cpSyntax "-" ; return NegE
  ]
...
l1mn ∷ QQ.QuasiQuoter
l1mn = QQ.QuasiQuoter quoteExpr (const $ HS.fail $ chars "quote pattern - I can't even") 
